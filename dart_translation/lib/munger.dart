...
class Munger{
  ...
  static final List<String> _mungableExtensions = [
    '.lua',  // only lua on Linux
    //'.tga',  '.mcfg', '.fx', '.prp', '.bnd', '.snd', '.mus',
    //'.combo', '.sanm', '.hud', '.cfg', '.pth', '.sky', '.lgt', '.pvs', '.tsr',
  ];

  static bool isMungableFile(String file) {
  ...
  static String ensureMungedFile(String fileName, [String platform = 'pc']) {
  ...
  static String? mungeFile(String file, [String platform = 'pc']) {
    final lastDot = file.lastIndexOf('.');
  ...
  static String? _textureMunge(String file, String platform) {
  ...
  static String getScriptMunge(){
    String retVal = "";
    String ext = Platform.isWindows ? '.exe' : '';
    String loc_1 = "./ScriptMunge$ext";
    String loc_2 = "./bin/ScriptMunge$ext";
    String loc_3 = pathJoin(modToolsDir, 'ToolsFL/bin/ScriptMunge$ext');
    if( File(loc_1).existsSync()) {
      retVal = loc_1;
    } else if( File(loc_2).existsSync()) {
      retVal = loc_2;
    } else if( File(loc_3).existsSync()) {
      retVal = loc_3;
    }else{
      print("Couldn't find 'ScriptMunge' program");
    }
    print("getScriptMunge(); using '$retVal'");
    return retVal;
  }

  static String? _scriptMunge(String file) {
    if (!File(file).existsSync()) return null;
    final sep = file.contains('/') ? '/' : '\\';
    final lastSep = file.lastIndexOf(sep);
    final tmpFile = pathJoin(tmpDir, lastSep >= 0 ? file.substring(lastSep + 1) : file);
  ...
  static String? _configMunge(String file) {
    if (!File(file).existsSync()) return null;
    final sep = file.contains('/') ? '/' : '\\';
    final lastSep = file.lastIndexOf(sep);
    final tmpFile = pathJoin(tmpDir, lastSep >= 0 ? file.substring(lastSep + 1) : file);
  ...
  static String getConfigMungeArgs(String fileName) {
